...
test("deploy workflow refreshes content and publishes a Pages artifact", async () => {
  ...
  assert.equal(
    findStep(steps, "Set up Python").uses,
  ...
  assert.match(
    findStep(steps, "Generate browser search indexes").run,
    /index:pages/,
  );
  assert.match(findStep(steps, "Prepare Pages artifact").run, /\.nojekyll/);
  assert.equal(
    findStep(steps, "Configure GitHub Pages").uses,
    "actions/configure-pages@v5",
  );
  assert.equal(
    findStep(steps, "Upload Pages artifact").uses,
