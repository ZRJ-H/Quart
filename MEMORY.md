...
- **证据约束**: 模型只可基于采集到的标题、正文摘要、作者、指标和评论生成；优先使用已配置的 DeepSeek/GO，缺省使用 `GITHUB_TOKEN` + GitHub Models，结构、长度或调用失败时回退到满足同等阅读预算的 `evidence-only` 摘要。
...
- 接入 GitHub Models（`models: read`）作为免配置默认摘要器，并为无模型降级版加入长度预算与长文本截断。
